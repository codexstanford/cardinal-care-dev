...
let progress_button_conditions : (() => boolean)[] = [
    () => {return (document.getElementById("username-input-field") as HTMLInputElement).value !== "";},
...
function addAllEventListeners() : void {
    ...
    username_question_button.addEventListener("click", function () {
        let username_input = document.getElementById("username-input-field") as HTMLInputElement;
        let username_signin_input = document.getElementById("username-signin-input-field") as HTMLInputElement;
...
function updateProgressQuestionButtons() {
    ...
    for (let i = 0; i < progress_question_buttons.length; i++) {
        let progress_question_button = progress_question_buttons[i] as HTMLElement;
...
function validEmailEntered() : boolean {
    let email_input_string = (document.getElementById("email-input-field") as HTMLInputElement).value;

    return isValidEmailString(email_input_string);
}

// Courtesy of ChatGPT
function isValidEmailString(email_string: string): boolean {
    // Regular expression for email validation
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return emailRegex.test(email_string);
}

function usernameAndPasswordEntered() : boolean {
    return ((document.getElementById("username-signin-input-field") as HTMLInputElement).value !== "") &&
